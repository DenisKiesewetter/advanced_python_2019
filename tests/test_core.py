...
def test_find_black_peaks():
    peaks = playground.core.find_black_peaks([(1,1,1),(1,0,1),(1,1,1)])
    assert peaks == [2]  
    
def test_find_black_two_peaks():
    peaks = playground.core.find_black_peaks([(1,1,1),(1,0,1),(1,1,1),(0,1,1),(1,1,1)])
    assert peaks == [2, 2] 

def test_find_black_peaks_max_edge():
    peaks = playground.core.find_black_peaks([(1,0,0),(1,0,1),(1,1,0)])
    assert peaks == [] 
    
def test_find_black_peaks_empty_list():
    peaks = playground.core.find_black_peaks([])
    assert peaks == [] 
